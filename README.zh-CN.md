...
<p align="center">
  <a href="https://hoocowork.app">HooCowork</a> · <a href="https://hoocowork.app/docs">文档</a> · <a href="https://discord.gg/buxwujPNRE">Discord</a> · <a href="https://github.com/kolisachint/hoocowork/issues">Bug 报告</a> · <a href="CONTRIBUTING.md">贡献指南</a>
</p>
...
- **插件系统** - 通过自定义选项卡、后端服务与集成扩展 HooCowork。 [开始构建 →](https://github.com/kolisachint/hoocowork-plugin-starter)
...
**[立即开始 HooCowork](https://hoocowork.app)**

### 自托管（开源）

#### npm

启动 HooCowork，只需一行 `npx`（需要 Node.js v22+）：
...
更多配置选项、PM2、远程服务器设置等，请参阅 **[文档 →](https://hoocowork.app/docs)**。
...
HooCowork 是 HooCowork 的开源 UI 层。你可以在本地机器上自托管它，也可以使用提供团队功能与深入集成的 HooCowork。

| | HooCowork（自托管） | HooCowork |
|---|---|---|
| **适合对象** | 需要为本地代理会话提供完整 UI 的开发者 | 需要部署在云端，随时从任何地方访问代理的团队与开发者 |
| **访问方式** | 通过 `[yourip]:port` 在浏览器中访问 | 浏览器、任意 IDE、REST API、n8n |
| **设置** | `npx @kolisachint/hoocowork` | 无需设置 |
| **机器需保持开机吗** | 是 | 否 |
...
> 两种方式都使用你自己的 AI 订阅（Claude、Cursor 等）— HooCowork 提供环境，而非 AI。
...
HooCowork 配备插件系统，允许你添加带自定义前端 UI 和可选 Node.js 后端的选项卡。在 Settings > Plugins 中直接从 Git 仓库安装插件，或自行开发。

### 可用插件

| 插件 | 描述 |
|---|---|
| **[Project Stats](https://github.com/kolisachint/hoocowork-plugin-starter)** | 展示当前项目的文件数、代码行数、文件类型分布、最大文件以及最近修改的文件 |

### 自行构建

**[Plugin Starter Template →](https://github.com/kolisachint/hoocowork-plugin-starter)** — Fork 该仓库以构建自己的插件。示例包括前端渲染、实时上下文更新和 RPC 通信。

**[插件文档 →](https://hoocowork.app/docs/plugin-overview)** — 提供插件 API、清单格式、安全模型等完整指南。
...
HooCowork 与 HooCowork 是对 Claude Code 的扩展，而非旁观 — MCP 服务器、权限、设置、会话与 Claude Code 完全一致。

- **覆盖全部会话** — HooCowork 会自动扫描 `~/.claude` 文件夹中的每个会话。Remote Control 只暴露当前活动的会话。
- **设置统一** — 在 HooCowork 中修改的 MCP、工具权限等设置会立即写入 Claude Code。
- **支持更多 Agents** — Claude Code、Cursor CLI、Codex、Gemini CLI。
- **完整 UI** — 除了聊天界面，还包括文件浏览器、Git 集成、MCP 管理和 Shell 终端。
- **HooCowork 保持运行于云端** — 关闭本地设备也不会中断代理运行，无需监控终端。
...
需要。HooCowork 只提供环境。你仍需自行获取 Claude、Cursor、Codex 或 Gemini 订阅。HooCowork 从 $7/月起提供托管环境。

</details>

<details>
<summary>能在手机上使用 HooCowork 吗？</summary>

可以。自托管时，在你的设备上运行服务器，然后在网络中的任意浏览器打开 `[yourip]:port`。HooCowork 可从任意设备访问，内置原生应用也在开发中。
...
会的。自托管模式下，HooCowork 读取并写入 Claude Code 使用的 `~/.claude` 配置。通过 UI 添加的 MCP 服务器会立即在 Claude Code 中可见。
...
- **[文档](https://hoocowork.app/docs)** — 安装、配置、功能与故障排除指南
- **[Discord](https://discord.gg/buxwujPNRE)** — 获取帮助并与社区交流
- **[GitHub Issues](https://github.com/kolisachint/hoocowork/issues)** — 报告 Bug 与建议功能
