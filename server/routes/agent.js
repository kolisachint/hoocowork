 ...
 *
 * @param {string} provider - (Optional) AI provider to use. Options: 'claude' | 'cursor' | 'codex' | 'gemini' | 'pi'
 *                           Default: 'claude'
 *
 ...
 *   - provider must be 'claude', 'cursor', 'codex', 'gemini', or 'pi'
...
router.post('/', validateExternalApiKey, async (req, res) => {
  ...
  if (!['claude', 'cursor', 'codex', 'gemini', 'pi'].includes(provider)) {
    return res.status(400).json({ error: 'provider must be "claude", "cursor", "codex", "gemini", or "pi"' });
  }
