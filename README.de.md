...
<p align="center">
  <a href="https://hoocowork.app">HooCowork</a> · <a href="https://hoocowork.app/docs">Dokumentation</a> · <a href="https://discord.gg/buxwujPNRE">Discord</a> · <a href="https://github.com/kolisachint/hoocowork/issues">Fehler melden</a> · <a href="CONTRIBUTING.md">Mitwirken</a>
</p>
...
- **Plugin-System** – HooCowork mit eigenen Plugins erweitern – neue Tabs, Backend-Dienste und Integrationen hinzufügen. [Eigenes Plugin erstellen →](https://github.com/kolisachint/hoocowork-plugin-starter)
...
**[Mit HooCowork starten](https://hoocowork.app)**
...
HooCowork sofort mit **npx** ausprobieren (erfordert **Node.js** v22+):
...
Die **[Dokumentation →](https://hoocowork.app/docs)** enthält weitere Konfigurationsoptionen, PM2, Remote-Server-Einrichtung und mehr.
...
> CI/CD verwendet bun für schnellere Builds. Installiere bun von https://bun.sh

#### Entwicklung

Für die lokale Entwicklung:
- `npm run dev` - Starte Frontend und Backend im Entwicklungsmodus
- `npm run build` oder `bun run build` - Production-Build
- `npm test` oder `bun test` - Tests ausführen

Sowohl `package-lock.json` als auch `bun.lock` werden im Repository gepflegt.

---

## Welche Option passt zu dir?

HooCowork ist die Open-Source-UI-Schicht, die HooCowork antreibt. Du kannst es auf deinem eigenen Rechner selbst hosten oder HooCowork nutzen, das darauf aufbaut und eine vollständig verwaltete Cloud-Umgebung, Team-Funktionen und tiefere Integrationen bietet.
...
| **Einrichtung** | `npx @kolisachint/hoocowork` | Keine Einrichtung erforderlich |
...
> Beide Optionen verwenden deine eigenen KI-Abonnements (Claude, Cursor usw.) – HooCowork stellt die Umgebung bereit, nicht die KI.
...
HooCowork verfügt über ein Plugin-System, mit dem benutzerdefinierte Tabs mit eigener Frontend-UI und optionalem Node.js-Backend hinzugefügt werden können. Plugins können direkt in **Einstellungen > Plugins** aus Git-Repos installiert oder selbst entwickelt werden.

### Verfügbare Plugins

| Plugin | Beschreibung |
|---|---|
| **[Project Stats](https://github.com/kolisachint/hoocowork-plugin-starter)** | Zeigt Dateianzahl, Codezeilen, Dateityp-Aufschlüsselung, größte Dateien und zuletzt geänderte Dateien des aktuellen Projekts |

### Eigenes Plugin erstellen

**[Plugin-Starter-Vorlage →](https://github.com/kolisachint/hoocowork-plugin-starter)** – Forke dieses Repository, um ein eigenes Plugin zu erstellen. Es enthält ein funktionierendes Beispiel mit Frontend-Rendering, Live-Kontext-Updates und RPC-Kommunikation zu einem Backend-Server.

**[Plugin-Dokumentation →](https://hoocowork.app/docs/plugin-overview)** – Vollständige Anleitung zur Plugin-API, zum Manifest-Format, zum Sicherheitsmodell und mehr.
...
HooCowork und HooCowork erweitern Claude Code, anstatt neben ihm zu laufen – MCP-Server, Berechtigungen, Einstellungen und Sitzungen sind exakt dieselben, die Claude Code nativ verwendet. Nichts wird dupliziert oder separat verwaltet.

Das bedeutet in der Praxis:

- **Alle Sitzungen, nicht nur eine** – HooCowork erkennt automatisch jede Sitzung aus dem `~/.claude`-Ordner. Remote Control stellt nur die einzelne aktive Sitzung bereit, um sie in der Claude Mobile App verfügbar zu machen.
- **Deine Einstellungen sind deine Einstellungen** – MCP-Server, Tool-Berechtigungen und Projektkonfiguration, die in HooCowork geändert werden, werden direkt in die Claude Code-Konfiguration geschrieben und treten sofort in Kraft – und umgekehrt.
...
- **HooCowork läuft in der Cloud** – Laptop zuklappen, der Agent läuft weiter. Kein Terminal zu überwachen, kein Rechner, der laufen muss.
...
Ja. HooCowork stellt die Umgebung bereit, nicht die KI. Du bringst dein eigenes Claude-, Cursor-, Codex- oder Gemini-Abonnement mit. HooCowork beginnt bei $7/Monat für die gehostete Umgebung zusätzlich dazu.

</details>

<details>
<summary>Kann ich HooCowork auf meinem Smartphone nutzen?</summary>

Ja. Bei Self-Hosted: Server auf dem eigenen Rechner starten und `[deineIP]:port` in einem beliebigen Browser im Netzwerk öffnen. Bei HooCowork: Von jedem Gerät aus öffnen – kein VPN, keine Portweiterleitung, keine Einrichtung. Eine native App ist ebenfalls in Entwicklung.
...
Ja, bei Self-Hosted. HooCowork liest aus und schreibt in dieselbe `~/.claude`-Konfiguration, die Claude Code nativ verwendet. MCP-Server, die über die UI hinzugefügt werden, erscheinen sofort in Claude Code und umgekehrt.
...
- **[Dokumentation](https://hoocowork.app/docs)** — Installation, Konfiguration, Funktionen und Fehlerbehebung
- **[Discord](https://discord.gg/buxwujPNRE)** — Hilfe erhalten und mit anderen Nutzer:innen in Kontakt treten
- **[GitHub Issues](https://github.com/kolisachint/hoocowork/issues)** — Fehlerberichte und Feature-Anfragen
