...
# Auto-start HooCowork server in background if not already running.
# This script is sourced from ~/.bashrc on sandbox shell open.

if ! pgrep -f "server/index.js" > /dev/null 2>&1; then
  nohup hoocowork start --port 3001 > /tmp/hoocowork-ui.log 2>&1 &
  disown

  echo ""
  echo "  HooCowork is starting on port 3001..."
  echo ""
