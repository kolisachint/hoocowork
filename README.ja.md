<div align="center">
  ...
  <h1>HooCowork（別名 Claude Code UI）</h1>
...
<p align="center">
  <a href="https://hoocowork.app">HooCowork</a> · <a href="https://hoocowork.app/docs">ドキュメント</a> · <a href="https://discord.gg/buxwujPNRE">Discord</a> · <a href="https://github.com/kolisachint/hoocowork/issues">バグ報告</a> · <a href="CONTRIBUTING.md">コントリビュート</a>
</p>
...
- **プラグインシステム** - カスタムプラグインで HooCowork を拡張 — 新しいタブ、バックエンドサービス、連携を追加できます。[自分で構築する →](https://github.com/kolisachint/hoocowork-plugin-starter)
...
**[HooCowork を始める](https://hoocowork.app)**

### セルフホスト（オープンソース）

#### npm

**npx** で今すぐ HooCowork を試せます（**Node.js** v22+ が必要）：
...
より詳細な設定オプション、PM2、リモートサーバー設定などについては **[ドキュメントはこちら →](https://hoocowork.app/docs)** を参照してください。
...
> CI/CD は高速ビルドのために bun を使用しています。https://bun.sh から bun をインストールしてください

#### 開発

ローカル開発:
- `npm run dev` - フロントエンドとバックエンドを開発モードで起動
- `npm run build` または `bun run build` - プロダクションビルド
- `npm test` または `bun test` - テストの実行

`package-lock.json` と `bun.lock` の両方がリポジトリで管理されています。

---

## どちらの選択肢が適していますか？

HooCowork は、HooCowork を支えるオープンソースの UI レイヤーです。自分のマシンにセルフホストすることも、フルマネージドのクラウド環境、チーム機能、より深い統合を備えた HooCowork を使うこともできます。

| | HooCowork（セルフホスト） | HooCowork |
|---|---|---|
...
| **セットアップ** | `npx @kolisachint/hoocowork` | セットアップ不要 |
| **マシンの稼働継続** | はい | いいえ |
...
> どちらの選択肢でも、AI のサブスクリプション（Claude、Cursor など）はご自身のものを使用します — HooCowork が提供するのは環境であり、AI そのものではありません。
...
HooCowork にはプラグインシステムがあり、独自のフロントエンド UI と（必要に応じて）Node.js バックエンドを持つカスタムタブを追加できます。プラグインは **Settings > Plugins** から git リポジトリを直接指定してインストールするか、自作できます。

### 利用可能なプラグイン

| プラグイン | 説明 |
|---|---|
| **[Project Stats](https://github.com/kolisachint/hoocowork-plugin-starter)** | 現在のプロジェクトについて、ファイル数、コード行数、ファイル種別の内訳、最大ファイル、最近変更されたファイルを表示 |

### 自作する

**[Plugin Starter Template →](https://github.com/kolisachint/hoocowork-plugin-starter)** — このリポジトリを fork して独自プラグインを作れます。フロントエンド描画、ライブコンテキスト更新、バックエンドサーバーへの RPC 通信を含む動作例が入っています。

**[プラグインのドキュメント →](https://hoocowork.app/docs/plugin-overview)** — プラグイン API、manifest 形式、セキュリティモデルなどの完全ガイド。
...
HooCowork と HooCowork は、Claude Code の横に別物として存在するのではなく、Claude Code を拡張します — MCP サーバー、権限、設定、セッションは Claude Code がネイティブに使うものと完全に同一です。複製したり、別系統で管理したりしません。

- **すべてのセッションにアクセス** — HooCowork は `~/.claude` フォルダのすべてのセッションを自動検出します。Remote Control は、Claude モバイルアプリで利用可能にするため、1つのアクティブセッションだけを公開します。
- **設定はあなたの設定** — HooCowork で変更した MCP サーバー、ツール権限、プロジェクト構成は、Claude Code の設定に直接書き込まれて即座に反映され、その逆（Claude Code での変更が UI に反映）も同様です。
...
- **HooCowork はクラウド上で稼働** — ノートパソコンを閉じてもエージェントは動き続けます。監視が要る端末も、スリープ防止も不要です。
...
はい。HooCowork は環境を提供するものであり、AI は含まれません。Claude、Cursor、Codex、または Gemini のサブスクリプションはご自身でご用意ください。HooCowork のホスティング環境はそれに加えて月額 $7 から提供されます。

</details>

<details>
<summary>HooCowork をスマホで使えますか？</summary>

はい。セルフホストの場合は、自身のマシンでサーバーを起動し、ネットワーク内のブラウザで `[yourip]:port` を開いてください。HooCowork を使う場合は、任意のデバイスからアクセスできます。VPN もポートフォワーディングも不要で、セットアップも不要です。ネイティブアプリも開発中です。
...
はい、セルフホストの場合です。HooCowork は Claude Code がネイティブに使う `~/.claude` 設定を読み書きします。UI から追加した MCP サーバーは即座に Claude Code に反映され、その逆も同様です。
...
- **[ドキュメント](https://hoocowork.app/docs)** — インストール、設定、機能、トラブルシューティング
- **[Discord](https://discord.gg/buxwujPNRE)** — ヘルプを得たり、ユーザー同士で交流したりできます
- **[GitHub Issues](https://github.com/kolisachint/hoocowork/issues)** — バグ報告と機能要望
