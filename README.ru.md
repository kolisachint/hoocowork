...
<p align="center">
  <a href="https://hoocowork.app">HooCowork</a> · <a href="https://hoocowork.app/docs">Документация</a> · <a href="https://discord.gg/buxwujPNRE">Discord</a> · <a href="https://github.com/kolisachint/hoocowork/issues">Сообщить об ошибке</a> · <a href="CONTRIBUTING.md">Участие в разработке</a>
</p>
...
- **Система плагинов** - расширяйте HooCowork кастомными плагинами — добавляйте новые вкладки, бэкенд-сервисы и интеграции. [Создать свой →](https://github.com/kolisachint/hoocowork-plugin-starter)
...
**[Начать с HooCowork](https://hoocowork.app)**
...
Попробовать HooCowork можно сразу через **npx** (требуется **Node.js** v22+):
...
Посетите **[документацию →](https://hoocowork.app/docs)**, чтобы узнать про дополнительные варианты конфигурации, PM2, настройку удалённого сервера и многое другое.
...
> CI/CD использует bun для ускорения сборки. Установите bun с https://bun.sh

#### Разработка

Для локальной разработки:
- `npm run dev` - Запуск фронтенда и бэкенда в режиме разработки
- `npm run build` или `bun run build` - Production сборка
- `npm test` или `bun test` - Запуск тестов

В репозитории поддерживаются как `package-lock.json`, так и `bun.lock`.

---

## Какой вариант подходит вам?

HooCowork — это open source UI-слой, на котором построен HooCowork. Вы можете развернуть его на своей машине или использовать HooCowork, который добавляет полностью управляемую облачную среду, командные функции и более глубокие интеграции.
...
| **Настройка** | `npx @kolisachint/hoocowork` | Настройка не требуется |
...
> В обоих вариантах используются ваши собственные AI-подписки (Claude, Cursor и т.д.) — HooCowork предоставляет среду, а не сам AI.
...
У HooCowork есть система плагинов, которая позволяет добавлять кастомные вкладки со своим frontend UI и (опционально) Node.js бэкендом. Устанавливайте плагины напрямую из git-репозиториев в **Settings > Plugins** или создавайте свои.

### Доступные плагины

| Плагин | Описание |
|---|---|
| **[Project Stats](https://github.com/kolisachint/hoocowork-plugin-starter)** | Показывает количество файлов, строки кода, разбивку по типам файлов, самые большие файлы и недавно изменённые файлы для текущего проекта |

### Создать свой

**[Plugin Starter Template →](https://github.com/kolisachint/hoocowork-plugin-starter)** — сделайте форк этого репозитория, чтобы создать свой плагин. В шаблоне есть рабочий пример с рендерингом на фронтенде, live-обновлением контекста и RPC-коммуникацией с бэкенд-сервером.

**[Plugin Documentation →](https://hoocowork.app/docs/plugin-overview)** — полный гайд по plugin API, формату манифеста, модели безопасности и другому.
...
HooCowork и HooCowork расширяют Claude Code, а не работают рядом с ним — ваши MCP-серверы, разрешения, настройки и сессии остаются теми же самыми, что и в нативном Claude Code. Ничего не дублируется и не управляется отдельно.

Вот что это означает на практике:

- **Все ваши сессии, а не одна** — HooCowork автоматически находит каждую сессию из папки `~/.claude`. Remote Control предоставляет только одну активную сессию, чтобы сделать её доступной в мобильном приложении Claude.
- **Ваши настройки — это ваши настройки** — MCP-серверы, права инструментов и конфигурация проекта, изменённые в HooCowork, записываются напрямую в конфиг Claude Code и вступают в силу сразу же, и наоборот.
...
- **HooCowork работает в облаке** — закройте ноутбук, и агент продолжит работать. Не нужно следить за терминалом и держать машину постоянно активной.
...
Да. HooCowork предоставляет среду, а не сам AI. Вы приносите свою подписку Claude, Cursor, Codex или Gemini. HooCowork начинается от $7/месяц за хостируемую среду поверх этого.

</details>

<details>
<summary>Можно ли пользоваться HooCowork с телефона?</summary>

Да. Для self-hosted запустите сервер на своей машине и откройте `[yourip]:port` в любом браузере в вашей сети. Для HooCowork откройте сервис с любого устройства — без VPN, проброса портов и дополнительной настройки. Нативное приложение тоже в разработке.
...
Да, в self-hosted режиме. HooCowork читает и записывает тот же конфиг `~/.claude`, который Claude Code использует нативно. MCP-серверы, добавленные через UI, сразу появляются в Claude Code, и наоборот.
...
- **[Документация](https://hoocowork.app/docs)** — установка, настройка, возможности и устранение неполадок
- **[Discord](https://discord.gg/buxwujPNRE)** — помощь и общение с другими пользователями
- **[GitHub Issues](https://github.com/kolisachint/hoocowork/issues)** — сообщения об ошибках и запросы новых функций
