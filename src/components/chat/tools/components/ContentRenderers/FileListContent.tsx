        ...
        {files.map((file, index) => {
          ...
          return (
            <span key={index} className="inline-flex items-center">
              <button
                onClick={handleClick}
                className="font-mono text-[11px] text-[var(--brand-accent)] transition-colors hover:text-[var(--brand-accent)] hover:underline dark:text-[var(--brand-accent)] dark:hover:text-[var(--brand-accent)]"
                title={filePath}
              >
                {fileName}
              </button>
