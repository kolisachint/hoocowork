...
const getIndicatorConfig = (status: TaskIndicatorStatus): IndicatorConfig => {
  ...
  if (status === 'fully-configured') {
    return {
      icon: CheckCircle,
      colorClassName: 'text-[var(--ok)]',
      backgroundClassName: 'bg-[var(--ok)]/10',
  ...
  if (status === 'taskmaster-only') {
    return {
      icon: Settings,
      colorClassName: 'text-[var(--brand-accent)]',
      backgroundClassName: 'bg-[var(--brand-accent)]/10',
  ...
  if (status === 'mcp-only') {
    return {
      icon: AlertCircle,
      colorClassName: 'text-[var(--warn)]',
      backgroundClassName: 'bg-[var(--warn)]/10',
      label: 'MCP Ready',
